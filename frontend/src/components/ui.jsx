...
export function SheetHeader({ title, subtitle, badge, onClose }) {
  return (
    <div className="sheet-hdr">
      <div style={{ flex: 1, minWidth: 0 }}>
        <div className="row" style={{ gap: 8, alignItems: 'center' }}>
          <h3 className="sheet-title capitalize">{title}</h3>
          {badge && <span className="tag acc" style={{ fontSize: 11 }}>{badge}</span>}
        </div>
        {subtitle && <div className="sheet-hdr-sub">{subtitle}</div>}
      </div>
      {onClose && (
        <button
          className="iconbtn"
