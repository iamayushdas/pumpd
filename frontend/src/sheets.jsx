...
import { Button, Slider, Switch, Segmented, SelectRow, Row, SheetHeader } from './components/ui.jsx'
  ...
  return (
    <div style={{ textAlign: 'center', padding: '4px 0' }}>
      <div style={{
        ...
        height: 44,
        borderRadius: 12,
        background: danger ? 'color-mix(in srgb, var(--red) 16%, transparent)' : 'var(--acc-soft)',
        ...
        <Icon name={danger ? 'trash' : 'sparkles'} />
      </div>
      {title && <h3 style={{ margin: '0 0 8px', fontSize: 18, fontWeight: 700 }}>{title}</h3>}
      <div className="muted small" style={{ marginBottom: 18, lineHeight: 1.5 }}>{message}</div>
      <button className={'btn ' + (danger ? 'danger' : 'primary')} onClick={() => { close(); onConfirm && onConfirm() }}>
...
function BwSheet({ required, onDone, close }) {
  ...
  return <>
    <SheetHeader
      title={required ? t('Quick Check-in') : t('Log Body Weight')}
      subtitle={required ? t('Slide or tap to set your weight before training') : `${t('Today')}, ${fmtDate(todayISO(), true)}`}
      onClose={required ? null : close}
    />
...
function ImportSummary({ parsed, close }) {
  ...
  return <>
    <SheetHeader
      title={parsed.source ? t('Import from {0}', parsed.source) : t('Import History')}
      subtitle={parsed.from === parsed.to ? fmtDate(parsed.from, true) : fmtDate(parsed.from, true) + ' – ' + fmtDate(parsed.to, true)}
      onClose={close}
    />
...
function GoalSheet({ close }) {
  ...
  return <>
    <SheetHeader
      title={t('Target Goal Weight')}
      subtitle={t('Drawn as a reference line across all weight charts')}
      onClose={close}
    />
...
function ExerciseDetail({ ex, close }) {
  ...
  const instructions = instrFor(ex)

  return (
    <>
      <SheetHeader
        title={ex.n}
        subtitle={t(ex.tg || ex.bp)}
        onClose={close}
      />
      <Media ex={ex} />
      <div className="row" style={{ gap: 6, flexWrap: 'wrap', margin: '12px 0 10px' }}>
        <span className="tag acc capitalize">{t(ex.bp)}</span>
        {ex.tg && <span className="tag capitalize"><Icon name="target" />{t(ex.tg)}</span>}
        <span className="tag capitalize"><Icon name="dumbbell" />{t(ex.eq)}</span>
        {(ex.sm || []).slice(0, 3).map((s, i) => (
          <span key={i} className="tag capitalize">{t(s)}</span>
        ))}
      </div>
      ...
      {best > 0 && (
        <div className="stat-pr-banner" style={{ margin: '10px 0' }}>
          <span className="stat-pr-badge"><Icon name="trophy" /> {t('ALL-TIME BEST')}</span>
          <span className="stat-pr-val">{fmtNum(best)} {st.unit}</span>
        </div>
      )}

      <Button variant="primary" icon="plus" style={{ margin: '10px 0 6px' }} onClick={() => addToRoutineSheet(ex)}>
        {t('Add to my plan')}
      </Button>
      ...
      {instructions.length > 0 && (
        <div style={{ marginTop: 16 }}>
          <h4 className="sec" style={{ marginBottom: 10 }}>
            {t('Step-by-step Guide')}
            {!INSTR_LANGS.includes(getLang()) && <span className="dim"> · English</span>}
          </h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {instructions.map((s, i) => (
              <div key={i} className="instr-step">
                <span className="instr-step-num">{i + 1}</span>
                <span className="instr-step-txt">{s}</span>
              </div>
            ))}
          </div>
        </div>
...
function AddToRoutine({ ex, close }) {
  ...
  return <>
    <SheetHeader
      title={t('Add “{0}”', ex.n)}
      subtitle={t('Select a target routine or create a new one')}
      onClose={close}
    />
    <div className="list">
...
function CustomExForm({ existing, prefill, onDone, close }) {
  ...
  return <>
    <SheetHeader
      title={existing ? t('Edit Custom Exercise') : t('Create Custom Exercise')}
      subtitle={t('Name it and pick a body part')}
      onClose={close}
    />
...
function ExConfig({ ex, existing, onSave, onDelete, close, routine }) {
  ...
  return <>
    ...
    <div className="row" style={{ gap: 6, flexWrap: 'wrap', margin: '10px 0 14px' }}>
      ...
      <span className="tag capitalize">{t(ex.tg || ex.bp)}</span>
      <span className="tag capitalize">{t(ex.eq)}</span>
    </div>
    {ex.desc && <div className="exnote">{ex.desc}</div>}
...
export const glyphPicker = (current, onPick) => {
  ...
  return ui().openSheet(close => <>
    <SheetHeader
      ...
      subtitle={t('Custom glyph for your routine')}
      onClose={close}
    />
    {GLYPH_GROUPS.map(g => (
...
function PlanTools({ close }) {
  ...
  return <>
    <SheetHeader
      ...
      subtitle={t('Send your routines to a friend, or put your week on paper')}
      onClose={close}
    />
    <Button variant="primary" icon="upload" onClick={exportFile} disabled={!hasRoutines}>{t('Export plan file')}</Button>
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own pumpd — routines only, none of your workouts or weigh-ins.')}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
...
function PlanImport({ bundle, close }) {
  ...
  return <>
    <SheetHeader
      title={bundle.name ? t('Import “{0}”', bundle.name) : t('Import Plan')}
      subtitle={t(bundle.routineCount === 1 ? '{0} routine' : '{0} routines', bundle.routineCount)}
      onClose={close}
    />
...
function DayOverride({ iso, close }) {
  ...
  return <>
    <div className="row between" style={{ marginBottom: 4, alignItems: 'flex-start' }}>
      <div>
        <h3 style={{ margin: 0, fontSize: 22, fontWeight: 750 }}>{fmtDate(iso, true)}</h3>
        <div className="dim small" style={{ marginTop: 2 }}>
          {t('Adjust training session for this specific day')}
        </div>
      </div>
      {isToday && (
        <span className="tag acc" style={{ fontSize: 11, fontWeight: 600 }}>
          {t('Today')}
        </span>
      )}
    </div>

    <div className="sheet-banner" style={{ marginTop: 12 }}>
      <div className="row between">
        <span className="small muted" style={{ fontWeight: 500 }}>
          {t('Default Weekly Plan')}:
        </span>
        <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--label)' }}>
          {weeklyR ? weeklyR.name : t('Rest Day')}
        </span>
      </div>
      {hasOvr && (
        <div className="small row" style={{ color: 'var(--orange)', marginTop: 6, gap: 5, fontWeight: 500 }}>
          <Icon name="bolt" style={{ fontSize: 13 }} />
          <span>{t('Custom override active for this date')}</span>
        </div>
      )}
    </div>

    <div style={{ marginBottom: 14 }}>
      <div className="small muted" style={{ fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.03em', marginBottom: 8 }}>
        {t('SELECT ROUTINE')}
      </div>

      {st.routines.map(r => {
        const isSelected = effId === r.id
        return (
          <div
            key={r.id}
            className={`sheet-option-card ${isSelected ? 'selected' : ''}`}
            onClick={() => set(r.id)}
          >
            ...
            <div className="grow" style={{ minWidth: 0 }}>
              <div className="tt" style={{ fontWeight: 600, fontSize: 15 }}>{r.name}</div>
              <div className="ss" style={{ fontSize: 12.5, color: 'var(--label-2)' }}>{exCount(r.ex.length)}</div>
            </div>
            {isSelected ? (
              <span className="sheet-option-check">
                <Icon name="check" />
              </span>
            ) : (
      ...
      {/* Rest Day Option */}
      <div
        className={`sheet-option-card ${effId === null ? 'selected rest' : ''}`}
        onClick={() => set('rest')}
      >
        <div className="sheet-option-icon" style={{ color: 'var(--purple)' }}>
          <Icon name="moon" />
        </div>
        <div className="grow">
          <div className="tt" style={{ fontWeight: 600, fontSize: 15 }}>{t('Rest / Recovery Day')}</div>
          <div className="ss" style={{ fontSize: 12.5, color: 'var(--label-2)' }}>{t('Skip training and recover')}</div>
        </div>
        {effId === null ? (
          <span className="sheet-option-check rest">
            <Icon name="check" />
          </span>
        ) : (
          <Icon name="chevronRight" className="chev" style={{ fontSize: 14 }} />
      ...
      {/* Reset to Weekly Plan (if overridden) */}
      {hasOvr && (
        <div
        ...
        >
          <div className="sheet-option-icon" style={{ background: 'var(--surface-3)', color: 'var(--label)' }}>
            <Icon name="reset" />
          </div>
          <div className="grow">
            <div className="tt" style={{ fontWeight: 600, fontSize: 15 }}>{t('Reset to Weekly Plan')}</div>
            <div className="ss" style={{ fontSize: 12.5, color: 'var(--label-2)' }}>{t('Revert to {0}', weeklyR ? weeklyR.name : t('Rest'))}</div>
          </div>
          <Icon name="chevronRight" className="chev" style={{ fontSize: 14 }} />
        </div>
      )}
...
function DayAssign({ day, close }) {
  const st = useStore(s => s.S)
  const set = v => {
    update(s => { if (v) s.week[day] = v; else delete s.week[day] })
    close()
  }

  const currentRoutineId = st.week[day]

  return <>
    <div style={{ marginBottom: 12 }}>
      <h3 style={{ margin: 0, fontSize: 22, fontWeight: 750 }}>{t(DAYN[day])}</h3>
      <div className="dim small" style={{ marginTop: 2 }}>
        {t('Set the default recurring workout for every {0}', t(DAYN[day]))}
      </div>
    </div>

    <div style={{ marginBottom: 16 }}>
      <div className="small muted" style={{ fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.03em', marginBottom: 8 }}>
        {t('ASSIGN RECURRING ROUTINE')}
      </div>

      {st.routines.map(r => {
        const isSelected = currentRoutineId === r.id
        return (
          <div
            key={r.id}
            className={`sheet-option-card ${isSelected ? 'selected' : ''}`}
      ...
      {/* Rest Day Option */}
      <div
        className={`sheet-option-card ${!currentRoutineId ? 'selected rest' : ''}`}
        onClick={() => set('')}
      >
        <div className="sheet-option-icon" style={{ color: 'var(--purple)' }}>
          <Icon name="moon" />
        ...
        <div className="grow">
          <div className="tt" style={{ fontWeight: 600, fontSize: 15 }}>{t('Scheduled Rest Day')}</div>
          <div className="ss" style={{ fontSize: 12.5, color: 'var(--label-2)' }}>{t('No training scheduled')}</div>
        </div>
        {!currentRoutineId ? (
          <span className="sheet-option-check rest">
            <Icon name="check" />
...
function WorkoutDetail({ w, close }) {
  const st = useStore(s => s.S)
  const duration = durPart(w.end - w.start)
  ...
  return <>
    <SheetHeader
      title={w.name}
      subtitle={fmtDate(w.d, true)}
      onClose={close}
    />

    <div className="r-stats-bar" style={{ marginTop: 0, marginBottom: 14 }}>
      <div className="r-stat-cell">
        <span className="r-stat-num">{duration.join('') || '—'}</span>
        <span className="r-stat-lbl">{t('Duration')}</span>
      </div>
      <div className="r-stat-divider" />
      <div className="r-stat-cell">
        <span className="r-stat-num">{vol}</span>
        <span className="r-stat-lbl">{t('Volume')}</span>
      </div>
      <div className="r-stat-divider" />
      <div className="r-stat-cell">
        <span className="r-stat-num">{setsCount}</span>
        <span className="r-stat-lbl">{t('Sets Done')}</span>
      </div>
    </div>
    ...
    <div style={{ marginBottom: 16 }}>
      {w.entries.map((e, i) => {
        const ex = EXIDX[e.id]
        return <div key={i} className="row" style={{ marginBottom: 10, alignItems: 'flex-start' }}>
          {ex && <Thumb ex={ex} />}
    ...
    <Button variant="danger" icon="trash" onClick={() => confirmSheet({ title: t('Delete workout?'), message: t('This removes it from your history for good.'), confirmText: t('Delete'), danger: true, onConfirm: () => { update(s => { s.workouts = s.workouts.filter(x => x.id !== w.id) }); close(); toast(t('Workout deleted')) } })}>{t('Delete workout')}</Button>
  </>
}
...
function TopWeight({ entryIdx, close }) {
  ...
  return <>
    <SheetHeader
      title={t('{0} done', ex.n)}
      subtitle={t('Confirm your working weight for next time')}
      onClose={close}
    />
    <WeightInput value={v} setValue={setV} unit={st.unit} />
    <div style={{ height: 10 }} />
...
function FinishSummary({ w, prs, e1prs = [], close }) {
  ...
  return <div style={{ textAlign: 'center', padding: '6px 0' }}>
    ...
    <h3 style={{ margin: '0 0 4px', fontSize: 22, fontWeight: 750 }}>{t('Workout Complete!')}</h3>
    <div className="t-sub muted" style={{ marginBottom: 14 }}>{w.name} · {fmtDate(w.d, true)}</div>

    <div className="tiles" style={{ textAlign: 'left', marginBottom: 14 }}>
      <div className="tile"><div className="l"><Icon name="clock" />{t('Duration')}</div><div className="v" style={{ fontSize: '1.2rem' }}>{fmtDur(w.end - w.start)}</div></div>
      <div className="tile"><div className="l"><Icon name="chartLine" />{t('Volume')}</div><div className="v" style={{ fontSize: '1.2rem' }}>{fmtVol(w.vol, st.unit)}</div></div>
      <div className="tile"><div className="l"><Icon name="dumbbell" />{t('Sets')}</div><div className="v" style={{ fontSize: '1.2rem' }}>{setsDone(w)}</div></div>
      <div className="tile"><div className="l"><Icon name="trophy" />{t('PRs')}</div><div className="v" style={{ fontSize: '1.2rem', color: prs.length ? 'var(--yellow)' : 'inherit' }}>{prs.length || '0'}</div></div>
    </div>
    {(prs.length > 0 || e1prs.length > 0) && <div style={{ textAlign: 'left', marginBottom: 14, background: 'var(--surface-2)', padding: '10px 12px', borderRadius: 12 }}>
      {prs.map(id => <div key={id} className="small accent capitalize row" style={{ gap: 6, marginBottom: 4 }}><Icon name="trophy" style={{ color: 'var(--yellow)' }} /><b>{t('New PR:')}</b> {(EXIDX[id] || {}).n || id}</div>)}
      {e1prs.map(p => <div key={p.id} className="small accent capitalize row" style={{ gap: 6 }}><Icon name="chartLine" style={{ color: 'var(--teal)' }} /><b>{t('Best Est. 1RM:')}</b> {(EXIDX[p.id] || {}).n || p.id} · {fmtNum(p.est)} {st.unit}</div>)}
    </div>}
