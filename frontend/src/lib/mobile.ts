...
export async function nativeLoad(): Promise<any> {
  try {
...
export async function nativeSave(state: any): Promise<void> {
  try {
    const { Filesystem, Directory, Encoding } = await import('@capacitor/filesystem')
...
export async function syncReminder(S: any, interactive = false): Promise<boolean> {
  try {
    ...
    const notifications = Object.entries(S.week || {})
      .filter(([, rid]: any[]) => rid && (S.routines || []).some((x: any) => x.id === rid))
      .map(([day, rid]: any[]) => ({
        id: 100 + Number(day),
        title: t('Workout day'),
        body: t('{0} is on the plan today — let\'s go!', S.routines.find((x: any) => x.id === rid).name),
