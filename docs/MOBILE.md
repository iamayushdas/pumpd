...
pumpd ships in two flavors from the same codebase:
...
- **iOS:** a Mac with Xcode 15+ and CocoaPods (`brew install cocoapods`). A free Apple ID
  ...
  for App Store distribution, which pumpd doesn't do.
...
pumpd's mobile app is not on the Play Store or App Store, and that's a choice: no store
...
apksigner sign --ks my.keystore --ks-key-alias opengym --out pumpd.apk aligned.apk
```
...
- **License:** pumpd is AGPL-3.0, which by itself sits badly with app-store terms of
