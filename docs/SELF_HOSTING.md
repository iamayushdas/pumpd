...
pumpd is two small containers (a web server and an API) plus a folder of your data.
...
pumpd signs you in with **passkeys** (WebAuthn). Browsers enforce two rules:
...
passkey prompt won't appear. To use pumpd from your phone you need a real HTTPS hostname.
...
Put pumpd behind something that terminates TLS for a hostname you control, then point it at
...
pumpd only needs the browser to reach it over `https://gym.example.com`.
...
pumpd can push two kinds of alert to your phone/desktop, even when the app isn't open:
